...
这里笔者建议阅读白皮书价格预言机部分更加深入理解计算的原理和一些做法背后的原因。

在具体使用 TWAP 的时候，也不是直接获取一个价格，需要分别记录两次价格累加值，得到差值之后再除以时间差，用户可以根据需要决定相隔多久，时间越久越不容易被操纵，但是就不那么实时。
...
第 12 和 13 行，先乐观地将代币划转到接收者地址，之后再进行 K 值的校验。
...
第 24 行是 K 值校验，交易后的 K 值必须大于等于原来的 K 值。
...
可以看到 Router 合约的成员变量只有 factory 地址和 WETH 地址，`receive` 函数确保只有 WETH 能转入 ETH。`ensure` 修饰器确保交易没有过期，下面基本每个函数都要用到。
